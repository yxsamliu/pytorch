...
bool MakeTwoClassOp<float, HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( MakeTwoClassKernel, 
      ...
      0,
      context_.hip_stream(), 
      static_cast<const int>(N), X.data<float>(), Y->template mutable_data<float>());
  return true;
}
...
bool MakeTwoClassGradientOp<float, HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( MakeTwoClassGradientKernel, 
      ...
      0,
      context_.hip_stream(), 
      static_cast<const int>(N), dY.data<float>(), dX->template mutable_data<float>());
  return true;
}
...
bool WeightedSigmoidCrossEntropyWithLogitsOp<float, HIPContext>::
 ...
 hipLaunchKernelGGL( WeightedSigmoidCrossEntropyWithLogitsKernel, 
      ...
      0,
      context_.hip_stream(), 
      static_cast<const int>(outer_size), static_cast<const int>(inner_size), logits_ptr, targets_ptr, weights_ptr, out_ptr);
  return true;
}
...
//TODO(surya) Add full GPU/HIP support for the CrossEntropyOp
