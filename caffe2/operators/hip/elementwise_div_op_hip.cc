...
__global__ void ComputeDivAGradientHIPKernel(
...
__global__ void ComputeSimpleDivBGradientHIPKernel(
    const int size,
    const TGrad* dC,
    const TIn* B,
    const TOut* C,
...
__global__ void ComputeDivBGradientHIPKernel(
    const int outer_size,
    const int inner_size,
    const SimpleArray<int, D> C_strides,
...
template <typename TGrad, typename TIn, int D>
void ComputeDivAGradientHIPImpl(
    const int outer_size,
    const int inner_size,
    const int* C_dims,
    const int* B_dims,
 ...
 hipLaunchKernelGGL( ComputeDivAGradientHIPKernel<TGrad, TIn, D>
...
template <typename TGrad, typename TIn, typename TOut, int D>
void ComputeDivBGradientHIPImpl(
    const int outer_size,
    const int inner_size,
    const int* C_dims,
    const int* B_axes,
 ...
 hipLaunchKernelGGL( ComputeDivBGradientHIPKernel<TGrad, TIn, TOut, D>
      , dim3(::min(outer_size, CAFFE_MAXIMUM_NUM_BLOCKS)),
         dim3(CAFFE_HIP_NUM_THREADS),
         0,
         context->hip_stream(), 
...
void ComputeDivAGradientHIP(
  ...
  if (outer_size > 0 && inner_size > 0) {
    ...
    math::Set<TGrad, HIPContext>(outer_size, TGrad(0), dA, context);
  }
}

template <typename TGrad, typename TIn, typename TOut>
void ComputeDivBGradientHIP(
    const std::vector<int>& C_dims,
  ...
  if (outer_size > 0 && inner_size > 0) {
    ...
    math::Set<TGrad, HIPContext>(outer_size, TGrad(0), dB, context);
  }
}
...
bool DivFunctor<HIPContext>::Backward(
  ...
  if (A_dims == B_dims) {
   ...
   hipLaunchKernelGGL( ComputeSimpleDivBGradientHIPKernel<TGrad, TIn, TOut>
  ...
  ComputeDivBGradientHIP<TGrad, TIn, TOut>(
      C_broadcast_dims, B_axes, dC, B, C, dB, context);
  ComputeDivAGradientHIP<TGrad, TIn>(
...
class BinaryElementwiseWithArgsGradientOp<
    NumericTypes,
    HIPContext,
    BinaryFunctorWithDefaultCtor<DivFunctor<HIPContext>>,
    SameTypeAsInput,
    SameTypeAsInput>
    final : public Operator<HIPContext> {
 public:
 ...
 private:
  ...
  BinaryFunctorWithDefaultCtor<DivFunctor<HIPContext>> functor_;
};
