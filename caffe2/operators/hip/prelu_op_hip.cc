...
bool PReluOp<float, HIPContext>::RunOnDevice() {
  ...
  if (C_shared) {
   hipLaunchKernelGGL( PReluKernel, 
        ...
        0,
        context_.hip_stream(), static_cast<const int>(X.size()), Xdata, Wdata, Ydata);
    return true;
  }
  // non-shared case.
  switch (order_) {
    case StorageOrder::NCHW: {
     ...
     hipLaunchKernelGGL( PReluKernelNCHW, 
          ...
          dim3(CAFFE_HIP_NUM_THREADS),
          0,
          context_.hip_stream(), static_cast<const int>(N), static_cast<const int>(C), static_cast<const int>(dim), Xdata, Wdata, Ydata);
    ...
    case StorageOrder::NHWC: {
     hipLaunchKernelGGL( PReluKernelNHWC, 
          ...
          dim3(CAFFE_HIP_NUM_THREADS),
          0,
          context_.hip_stream(), static_cast<const int>(X.size()), static_cast<const int>(C), Xdata, Wdata, Ydata);
...
bool PReluGradientOp<float, HIPContext>::RunOnDevice() {
  ...
  if (C_shared) {
   hipLaunchKernelGGL( PReluSharedWGradientKernelNCHW, 
        ...
        dim3(CAFFE_HIP_NUM_THREADS),
        0,
        context_.hip_stream(), static_cast<const int>(X.size()), Xdata, dYdata, dWdata);
   hipLaunchKernelGGL( PReluGradientKernel, 
        dim3(CAFFE_GET_BLOCKS(X.size())),
        dim3(CAFFE_HIP_NUM_THREADS),
        0,
        context_.hip_stream(), static_cast<const int>(X.size()), Xdata, Wdata, dYdata, dXdata);
  ...
  switch (order_) {
    case StorageOrder::NCHW: {
     ...
     hipLaunchKernelGGL( PReluWGradientKernelNCHW, 
          ...
          dim3(CAFFE_HIP_NUM_THREADS),
          0,
          context_.hip_stream(), static_cast<const int>(C), static_cast<const int>(N), static_cast<const int>(X.size()), Xdata, dYdata, dWdata);
     hipLaunchKernelGGL( PReluGradientKernelNCHW, 
          dim3(CAFFE_GET_BLOCKS(X.size())),
          dim3(CAFFE_HIP_NUM_THREADS),
          0,
          context_.hip_stream(), static_cast<const int>(N), static_cast<const int>(C), static_cast<const int>(dim), Xdata, Wdata, dYdata, dXdata);
    ...
    case StorageOrder::NHWC: {
     ...
     hipLaunchKernelGGL( PReluGradientKernelNHWC, 
          ...
          dim3(CAFFE_HIP_NUM_THREADS),
          0,
          context_.hip_stream(), static_cast<const int>(X.size()), static_cast<const int>(C), Xdata, Wdata, dYdata, dXdata);
