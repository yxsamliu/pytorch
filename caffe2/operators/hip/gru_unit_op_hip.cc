...
__global__ void GRUUnitKernel(
  ...
  HIP_1D_KERNEL_LOOP(index, ND) {
    ...
    if (!valid) {
      ...
      T sigmoid_update = hip_sigmoid(update);
  ...
  HIP_1D_KERNEL_LOOP(index, ND) {
    ...
    if (!valid) {
      ...
      *output_diff = 0;
      ...
      const T u = hip_sigmoid(X_offset[1 * dim + d]);
...
void GRUUnit<float, HIPContext>(
 ...
 hipLaunchKernelGGL( GRUUnitKernel<float>
      , dim3(CAFFE_GET_BLOCKS(N * D)),
         ...
         0,
         context->hip_stream(), 
          static_cast<const int>(N * static_cast<const int>(D)), static_cast<const int>(D), static_cast<const int>(t), H_prev, X, seqLengths, drop_states, H);
}

template <>
void GRUUnitGradient<float, HIPContext>(
    int N,
    int D,
    int t,
    const float* H_prev,
...
REGISTER_HIP_OPERATOR(GRUUnit, GRUUnitOp<float, HIPContext>);
REGISTER_HIP_OPERATOR(GRUUnitGradient, GRUUnitGradientOp<float, HIPContext>);
}
