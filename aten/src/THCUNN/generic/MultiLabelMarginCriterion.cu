           ...
           int64_t reduction)
{
  ...
  if(input->dim() == 1)
  {
   ...
   hipLaunchKernelGGL( cunn_MultiLabelMarginCriterion_updateOutput_kernel<real, accreal>
            ...
            int64_t reduction)
{
  ...
  if(gradInput->dim() == 1)
  {
   ...
   hipLaunchKernelGGL( cunn_MultiLabelMarginCriterion_updateGradInput_kernel<real, accreal>
      , dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state), 
