...
__global__ void InvStdHIPKernel(
    const int size,
    const float epsilon,
    const float* var,
    float* rsig) {
...
template <>
bool GroupNormOp<float, HIPContext>::RunOnDeviceImpl(
    const int N,
    const int G,
    const int D,
    const int HxW,
 ...
 hipLaunchKernelGGL( InvStdHIPKernel, 
      dim3(CAFFE_GET_BLOCKS(N * G)),
      dim3(CAFFE_HIP_NUM_THREADS),
      0,
      context_.hip_stream(), N * G, epsilon_, rsig_data, rsig_data);
...
bool GroupNormGradientOp<float, HIPContext>::RunOnDeviceImpl(
  ...
  if (order_ == StorageOrder::NCHW) {
   ...
   hipLaunchKernelGGL( ComputeInternalGradientsHIPKernel<float, StorageOrder::NCHW>
        , dim3(std::min(N * G, CAFFE_MAXIMUM_NUM_BLOCKS)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
