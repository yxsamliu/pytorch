...
template <typename T>
__global__ void ComputeFusedParamsHIPKernel(
    const int N,
    const int G,
    const int D,
    const T* mu,
...
__global__ void GroupNormForwardHIPKernel(
    const int N,
    const int C,
    const int HxW,
    const T* X,
...
template <>
__global__ void GroupNormForwardHIPKernel<float, StorageOrder::NCHW>(
    const int N,
    const int C,
    const int HxW,
    const float* X,
...
template <>
__global__ void GroupNormForwardHIPKernel<float, StorageOrder::NHWC>(
    const int N,
    const int C,
    const int HxW,
    const float* X,
...
template <typename T, StorageOrder kOrder>
__global__ void ComputeInternalGradientsHIPKernel(
    const int N,
    const int G,
    const int D,
    const int HxW,
...
// dmu/dX = 1 / n
template <typename T, StorageOrder kOrder>
__global__ void GroupNormBackwardHIPKernel(
    const int size,
    const int G,
    const int D,
    const int HxW,
...
template <typename T, StorageOrder kOrder>
__global__ void GammaBetaBackwardHIPKernel(
    const int N,
    const int G,
    const int D,
    const int HxW,
...
template <>
void GroupNormOp<float, HIPContext>::ComputeFusedParams(
    const int N,
    const int G,
    const int D,
    const float* mu,
    ...
    float* scale,
    float* bias) {
 hipLaunchKernelGGL( ComputeFusedParamsHIPKernel<float>
      , dim3(::min(N * G, CAFFE_MAXIMUM_NUM_BLOCKS)),
         dim3(CAFFE_HIP_NUM_THREADS),
         0,
         context_.hip_stream(), N, G, D, mu, rsig, gamma, beta, scale, bias);
}

template <>
void GroupNormOp<float, HIPContext>::GroupNormForwardNCHW(
    const int N,
    const int C,
    const int HxW,
    const float* X,
    ...
    const float* bias,
    float* Y) {
 hipLaunchKernelGGL( GroupNormForwardHIPKernel<float, StorageOrder::NCHW>
      , dim3(::min(N * C, CAFFE_MAXIMUM_NUM_BLOCKS)),
         dim3(CAFFE_HIP_NUM_THREADS),
         0,
         context_.hip_stream(), N, C, HxW, X, scale, bias, Y);
}

template <>
void GroupNormOp<float, HIPContext>::GroupNormForwardNHWC(
    const int N,
    const int C,
    const int HxW,
    const float* X,
    ...
    const float* bias,
    float* Y) {
 hipLaunchKernelGGL( GroupNormForwardHIPKernel<float, StorageOrder::NHWC>
...
// then: Y = s * X + b
template <>
bool GroupNormGradientOp<float, HIPContext>::RunOnDeviceImpl(
    const int N,
    const int G,
    const int D,
    const int HxW,
  ...
  if (order_ == StorageOrder::NCHW) {
   ...
   hipLaunchKernelGGL( ComputeInternalGradientsHIPKernel<float, StorageOrder::NCHW>
        , dim3(::min(N * G, CAFFE_MAXIMUM_NUM_BLOCKS)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
    ...
    // Computes dL/dX.
   hipLaunchKernelGGL( GroupNormBackwardHIPKernel<float, StorageOrder::NCHW>
   ...
   hipLaunchKernelGGL( GammaBetaBackwardHIPKernel<float, StorageOrder::NCHW>
    ...
    // dL/ds = Sum(dL/dY * gamma * X)
    // dL/db = Sum(dL/dY * gamma)
   hipLaunchKernelGGL( ComputeInternalGradientsHIPKernel<float, StorageOrder::NHWC>
        , dim3(::min(N * G, CAFFE_MAXIMUM_NUM_BLOCKS)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
    ...
    // Computes dL/dX.
   hipLaunchKernelGGL( GroupNormBackwardHIPKernel<float, StorageOrder::NHWC>
        , dim3(CAFFE_GET_BLOCKS(size)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
    ...
    // Computes dL/dgamma and dL/dbeta.
   hipLaunchKernelGGL( GammaBetaBackwardHIPKernel<float, StorageOrder::NHWC>
        , dim3(::min(C, CAFFE_MAXIMUM_NUM_BLOCKS)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
...
REGISTER_HIP_OPERATOR(GroupNorm, GroupNormOp<float, HIPContext>);
REGISTER_HIP_OPERATOR(
    GroupNormGradient,
    GroupNormGradientOp<float, HIPContext>);
