...
template <>
void fp16_momentum_sgd_update<HIPContext>(
    int N,
    const at::Half* g,
    const at::Half* m,
    at::Half* ng,
  ...
  if (prop.major >= 6) {
    if (!fp32_update) {
     hipLaunchKernelGGL( FP16MomentumSGDKernel, 
     ...
     hipLaunchKernelGGL( FP16MomentumSGDFP32Kernel, 
          dim3(CAFFE_GET_BLOCKS(N / 2)),
          dim3(CAFFE_HIP_NUM_THREADS),
          0,
          context->hip_stream(), 
...
REGISTER_HIP_OPERATOR(
    ...
    FP16MomentumSGDUpdateOp<at::Half, HIPContext>);
