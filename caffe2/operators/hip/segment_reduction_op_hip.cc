...
class HIPSparseLengthsSumOp : public Operator<HIPContext> {
 public:
  ...
  bool DoRunWithType() {
    ...
    auto maxThreads =
        GetDeviceProperty(CaffeHipGetDevice()).maxThreadsPerBlock;
    if (SparseFused) {
      if (post <= maxThreads) {
        int multiple = ::min(maxThreads / post, 16);
        dim3 block(post, multiple);
        size_t smem = sizeof(T) * post * multiple;

        // calling cuda kernel with ExactBlock = true, Average = false
       hipLaunchKernelGGL( sparse_length_sum_kernel<T, IndexType, true, false>
            , dim3(len_length), dim3(block), smem, context_.hip_stream(), 
                in_data,
                out_data,
                ...
                static_cast<int>(len_length),
                static_cast<int>(dataToReduceSize));
      } else {
        // calling cuda kernel with ExactBlock = false, Average = false
       hipLaunchKernelGGL( sparse_length_sum_kernel<T, IndexType, false, false>
            , dim3(len_length), dim3(maxThreads), 0, context_.hip_stream(), 
                in_data,
                out_data,
                prefix_sum_length_data,
                indices,
      ...
      if (post <= maxThreads) {
       hipLaunchKernelGGL( length_sum_kernel<T, true, false>
            , dim3(len_length), dim3(post), 0, context_.hip_stream(), 
                in_data, out_data, prefix_sum_length_data, static_cast<int>(N), static_cast<int>(post), static_cast<int>(len_length));
      } else {
       hipLaunchKernelGGL( length_sum_kernel<T, true, false>
            , dim3(len_length), dim3(maxThreads), 0, context_.hip_stream(), 
                in_data, out_data, prefix_sum_length_data, static_cast<int>(N), static_cast<int>(post), static_cast<int>(len_length));
 ...
 private:
  ...
  Tensor inclusive_scan_buffer_{HIP};
  Tensor inclusive_scan_length_buffer_{HIP};
};

template <typename T, class Context = HIPContext, bool SparseFused = true>
class HIPSparseLengthsMeanOp : public Operator<HIPContext> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
  HIPSparseLengthsMeanOp(const OperatorDef& operator_def, Workspace* ws)
      : Operator<HIPContext>(operator_def, ws) {}

  ~HIPSparseLengthsMeanOp() {}
  ...
  bool DoRunWithType() {
    ...
    if (SparseFused) {
      if (post <= maxThreads) {
       ...
       hipLaunchKernelGGL( sparse_length_sum_kernel<T, IndexType, true, true>
            , dim3(len_length), dim3(block), smem, context_.hip_stream(), 
                in_data,
                out_data,
                prefix_sum_length_data,
       ...
       hipLaunchKernelGGL( sparse_length_sum_kernel<T, IndexType, false, true>
            , dim3(len_length), dim3(maxThreads), 0, context_.hip_stream(), 
                in_data,
                out_data,
                prefix_sum_length_data,
      ...
      if (post <= maxThreads) {
        // calling cuda kernel with ExactBlock = true, Average = true
       hipLaunchKernelGGL( length_sum_kernel<T, true, true>
            , dim3(len_length), dim3(post), 0, context_.hip_stream(), 
                in_data, out_data, prefix_sum_length_data, static_cast<int>(N), static_cast<int>(post), static_cast<int>(len_length));
      } else {
        // calling cuda kernel with ExactBlock = true, Average = true
...
template <typename T, class Context = HIPContext, bool SparseFused = true>
class HIPSparseLengthsMaxOp : public Operator<HIPContext> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
  HIPSparseLengthsMaxOp(const OperatorDef& operator_def, Workspace* ws)
      : Operator<HIPContext>(operator_def, ws) {}

  ~HIPSparseLengthsMaxOp() {}
  ...
  bool DoRunWithType() {
    ...
    if (SparseFused) {
      if (post <= maxThreads) {
       ...
       hipLaunchKernelGGL( sparse_length_max_kernel<T, IndexType, true>
            , dim3(len_length), dim3(block), smem, context_.hip_stream(), 
                in_data,
                out_data,
                prefix_sum_length_data,
       ...
       hipLaunchKernelGGL( sparse_length_max_kernel<T, IndexType, false>
            , dim3(len_length), dim3(maxThreads), 0, context_.hip_stream(), 
                in_data,
                out_data,
                prefix_sum_length_data,
      ...
      if (post <= maxThreads) {
       hipLaunchKernelGGL( length_max_kernel<T, true>
            , dim3(len_length), dim3(post), 0, context_.hip_stream(), 
                in_data,
                out_data,
                prefix_sum_length_data,
...
class HIPUnsortedSegmentSumOp : public Operator<HIPContext> {
  ...
  bool RunOnDevice() override {
    ...
    if (segment_ids.size() > 4096) {
     ...
     hipLaunchKernelGGL( MaxSegmentKernel<SIndex>
          , dim3(1), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
 ...
 private:
  Tensor buffer_tensor_{HIP};
  Tensor K_tensor_{HIP};
  Tensor scaling_factors_{HIP}; // for mean
};
...
class SortedSegmentRangeMeanOp : public Operator<Context> {
 ...
 private:
  Tensor segment_len_{HIP}; // for mean
  Tensor segment_len_prefix_sum_{HIP};
  Tensor prefix_buffer_{HIP};
};
...
REGISTER_HIP_OPERATOR_STR(
    "LengthsSum",
    HIPSparseLengthsSumOp<float, HIPContext, false>);
REGISTER_HIP_OPERATOR_STR(
    "SparseLengthsSum",
    HIPSparseLengthsSumOp<float, HIPContext, true>);
REGISTER_HIP_OPERATOR_STR(
    "LengthsMean",
    HIPSparseLengthsMeanOp<float, HIPContext, false>);
REGISTER_HIP_OPERATOR_STR(
    "SparseLengthsMean",
    HIPSparseLengthsMeanOp<float, HIPContext, true>);
REGISTER_HIP_OPERATOR_STR(
    "LengthsMax",
    HIPSparseLengthsMaxOp<float, HIPContext, false>);
REGISTER_HIP_OPERATOR_STR(
    "SparseLengthsMax",
    HIPSparseLengthsMaxOp<float, HIPContext, true>);
REGISTER_HIP_OPERATOR_STR(
    "SparseLengthsWeightedSum",
    HIPSparseLengthsWeightedSumOp<float, HIPContext, true>);
REGISTER_HIP_OPERATOR_STR(
    "UnsortedSegmentSum",
    HIPUnsortedSegmentSumOp<float, int, false>);
REGISTER_HIP_OPERATOR_STR(
    "UnsortedSegmentMean",
    HIPUnsortedSegmentSumOp<float, int, true>);
REGISTER_HIP_OPERATOR_STR(
 ...
 public:
  ...
  bool RunOnDevice() override {
    ...
    if (post <= maxThreads) {
      ...
      // calling cuda kernel with ExactBlock = true, Average = false
     hipLaunchKernelGGL( length_sum_gradient_kernel<T, true, false>
          , dim3(len_length), dim3(block), 0, context_.hip_stream(), 

              in_data, out_data, prefix_sum_length_data, static_cast<int>(N), static_cast<int>(post), static_cast<int>(len_length));
    } else {
      // calling cuda kernel with ExactBlock = false, Average = false
     hipLaunchKernelGGL( length_sum_gradient_kernel<T, false, false>
          , dim3(len_length), dim3(maxThreads), 0, context_.hip_stream(), 
              in_data, out_data, prefix_sum_length_data, static_cast<int>(N), static_cast<int>(post), static_cast<int>(len_length));
    }
...
template <typename T, class Context = HIPContext>
class HIPSparseLengthsMeanGradientWithIndicesOp
    : public Operator<HIPContext> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
  HIPSparseLengthsMeanGradientWithIndicesOp(
      const OperatorDef& operator_def,
      Workspace* ws)
      : Operator<HIPContext>(operator_def, ws) {}
  ...
  bool RunOnDevice() override {
    ...
    if (post <= maxThreads) {
      ...
      // calling cuda kernel with ExactBlock = true, Average = true
     hipLaunchKernelGGL( length_sum_gradient_kernel<T, true, true>
          , dim3(len_length), dim3(block), 0, context_.hip_stream(), 

              in_data, out_data, prefix_sum_length_data, static_cast<int>(N), static_cast<int>(post), static_cast<int>(len_length));
    } else {
      // calling cuda kernel with ExactBlock = false, Average = true
     hipLaunchKernelGGL( length_sum_gradient_kernel<T, false, true>
          , dim3(len_length), dim3(maxThreads), 0, context_.hip_stream(), 
              in_data, out_data, prefix_sum_length_data, static_cast<int>(N), static_cast<int>(post), static_cast<int>(len_length));
    }
...
template <typename T, class Context = HIPContext>
class HIPSparseLengthsWeightedSumGradientWithIndicesOp
    : public Operator<HIPContext> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
...
template <typename T, class Context = HIPContext>
class HIPLengthsMaxWithMainInputAndForwardOutputGradientOp
    : public Operator<HIPContext> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
  HIPLengthsMaxWithMainInputAndForwardOutputGradientOp(
      const OperatorDef& operator_def,
      Workspace* ws)
      : Operator<HIPContext>(operator_def, ws) {}
  ...
  bool DoRunWithType() {
    ...
    if (post <= maxThreads) {
     ...
     hipLaunchKernelGGL( length_max_gradient_kernel<T, true>
          , dim3(len_length), dim3(block), 0, context_.hip_stream(), 
     ...
     hipLaunchKernelGGL( length_max_gradient_kernel<T, false>
          , dim3(len_length), dim3(maxThreads), 0, context_.hip_stream(), 
              in_data,
              out_data,
              dataInput.template data<T>(),
...
class HIPSparseLengthsIndicesInGradientWeightedSumWithMainInputGradientOp
 ...
 public:
  ...
  bool DoRunWithType() {
    ...
    if (post > 128) {
     hipLaunchKernelGGL( length_weighted_sum_with_main_input_gradient_kernel<T, IndexType, 512>
          , dim3(len_length), dim3(512), 0, context_.hip_stream(), 
              in_grads,
              in_weights,
              in_data,
              indices,
     ...
     hipLaunchKernelGGL( length_weighted_sum_with_main_input_gradient_kernel<T, IndexType, 128>
          , dim3(len_length), dim3(128), 0, context_.hip_stream(), 
              in_grads,
              in_weights,
              in_data,
              indices,
     ...
     hipLaunchKernelGGL( length_weighted_sum_with_main_input_gradient_kernel<T, IndexType, 64>
          , dim3(len_length), dim3(64), 0, context_.hip_stream(), 
              in_grads,
              in_weights,
              in_data,
              indices,
     ...
     hipLaunchKernelGGL( length_weighted_sum_with_main_input_gradient_kernel<T, IndexType, 32>
          , dim3(len_length), dim3(32), 0, context_.hip_stream(), 
              in_grads,
              in_weights,
              in_data,
              indices,
...
REGISTER_HIP_OPERATOR_STR(
    ...
    HIPLengthsMaxWithMainInputAndForwardOutputGradientOp<float, HIPContext>);
...
REGISTER_HIP_OPERATOR(
    ...
    HIPSparseLengthsMeanGradientWithIndicesOp<float, HIPContext>);
