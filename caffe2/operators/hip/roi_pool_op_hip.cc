...
template <>
bool RoIPoolOp<float, HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( ROIPoolForward<float>
      , dim3(CAFFE_GET_BLOCKS(output_size)),
         ...
         context_.hip_stream(), 
          ...
          static_cast<const int>(pooled_height_),
          static_cast<const int>(pooled_width_),
          R.data<float>(),
          Y->template mutable_data<float>(),
          argmax_data);
  return true;
...
template <>
bool RoIPoolGradientOp<float, HIPContext>::RunOnDevice() {
  auto& X = Input(0); // Input data to pool
  auto& R = Input(1); // RoIs
   ...
   hipLaunchKernelGGL( ROIPoolBackward<float>
        , dim3(CAFFE_GET_BLOCKS(dY.size())),
           ...
           context_.hip_stream(), 
            ...
            dY.data<float>(),
            A.data<int>(),
            static_cast<const int>(R.dim32(0)),
            spatial_scale_,
            static_cast<const int>(X.dim32(1)),
            static_cast<const int>(X.dim32(2)),
            static_cast<const int>(X.dim32(3)),
...
REGISTER_HIP_OPERATOR(RoIPool, RoIPoolOp<float, HIPContext>);
REGISTER_HIP_OPERATOR(RoIPoolGradient, RoIPoolGradientOp<float, HIPContext>);
