           ...
           int padT, int padW, int padH)
{
  ...
  while (totalZ > 0) {
   ...
   hipLaunchKernelGGL( cuda_VolumetricMaxUnpooling_updateOutput<real>, dim3(grid), dim3(block),
           ...
           int padT, int padW, int padH)
{
  ...
  while (totalZ > 0) {
   ...
   hipLaunchKernelGGL( cuda_VolumetricMaxUnpooling_updateGradInput<real>, dim3(grid), dim3(block),
      0, THCState_getCurrentStream(state), 
                                             gradOutputData,
                                             static_cast<int>(outputTime), static_cast<int>(outputHeight), static_cast<int>(outputWidth),
                                             cudaIndices,
