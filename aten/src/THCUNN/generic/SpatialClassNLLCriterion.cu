           ...
           int64_t ignore_index)
{
  ...
  if (reduction == Reduction::None) {
   ...
   hipLaunchKernelGGL( SpatialClassNLLCriterion_updateOutput_no_reduce_kernel<real>
 ...
 hipLaunchKernelGGL( cunn_SpatialClassNLLCriterion_updateOutput_kernel<real, accreal>
  ...
  if (reduction == Reduction::ElementwiseMean) {
   hipLaunchKernelGGL( cunn_SpatialClassNLLCriterion_sizeAverage_kernel<real>, dim3(1), dim3(1), 0, THCState_getCurrentStream(state), 
           ...
           int64_t ignore_index)
{
  ...
  if (reduction == Reduction::None) {
    ...
    int64_t count = batch_size * H * W;
   hipLaunchKernelGGL( SpatialClassNLLCriterion_updateGradInput_no_reduce_kernel<real>
      , dim3(GET_BLOCKS(count)), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
        static_cast<int64_t>(count),
        toDeviceTensor<THCIndex_t, 3>(state, target),
 ...
 hipLaunchKernelGGL( cunn_SpatialClassNLLCriterion_updateGradInput_kernel<real>
    , dim3(total_blocks), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
      gradInput_data,
      gradOutput_data,
      static_cast<THCIndex_t *>(target_data),
