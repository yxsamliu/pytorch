#include "caffe2/operators/rnn/hip/recurrent_network_executor_hip.h"
...
void HIPRecurrentNetworkExecutor::_ExecRange(int from, int to) {
  ...
  for (int t = from; t != to; t += direction) {
    ...
    for (int i = 0; i < ops.size(); i++) {
      ...
      if (gpu_id == -1 && rnn_op.op->device_option().device_type() == DeviceType::HIP) {
  ...
  for (int stream_id = 0; stream_id <= std::min(stream_seq, max_streams - 1);
    ...
    HIP_CHECK(
        hipStreamSynchronize(HIPContext::hip_stream(gpu_id, stream_id)));
  }
}
