...
template <typename T>
__global__ void ComputeFusedParamHIPKernel(
    const int C,
    const T epsilon,
    const T* scale,
    const T* bias,
...
template <>
__global__ void ComputeFusedParamHIPKernel<float>(
    const int C,
    const float epsilon,
    const float* scale,
    const float* bias,
...
template <typename T>
__global__ void ComputeBatchMomentsHIPKernel(
    const int C,
    const T scale,
...
template <typename T>
__global__ void ComputeRunningMomentsAndFusedParamHIPKernel(
    const int C,
    const T momentum,
    const T epsilon,
    const T* scale,
...
template <>
__global__ void ComputeRunningMomentsAndFusedParamHIPKernel<float>(
    const int C,
...
template <typename T>
__global__ void ComputeMultiBatchScaleBiasGradientsAndFusedParamsHIPKernel(
    const int C,
...
__global__ void ComputeScaleBiasGradientsAndFusedParamsHIPKernel(
    const int N,
    const int C,
    const int HxW,
    const T* dY,
...
template <typename T, StorageOrder kOrder>
__global__ void ComputeXGradientHIPKernel(
    const int size,
    const int C,
    const int HxW,
    const T* dY,
...
template <>
template <typename T>
void SpatialBNOp<HIPContext>::ComputeFusedParam(
    const int C,
    const T* scale,
    const T* bias,
    const T* mean,
    ...
    T* alpha,
    T* beta) {
 hipLaunchKernelGGL( ComputeFusedParamHIPKernel<T>
...
template <>
template <typename T>
void SpatialBNOp<HIPContext>::ComputeBatchMoments(
    const int N,
    const int C,
    const int HxW,
    const T* batch_mean_sum,
 ...
 hipLaunchKernelGGL( ComputeBatchMomentsHIPKernel<T>
      , dim3(CAFFE_GET_BLOCKS(C)),
         dim3(CAFFE_HIP_NUM_THREADS),
         0,
         context_.hip_stream(), 
...
template <>
template <typename T>
void SpatialBNOp<HIPContext>::ComputeRunningMomentsAndFusedParam(
    const int C,
    const T* scale,
    const T* bias,
    const T* mean,
    ...
    T* alpha,
    T* beta) {
 hipLaunchKernelGGL( ComputeRunningMomentsAndFusedParamHIPKernel<T>
      , dim3(CAFFE_GET_BLOCKS(C)),
         dim3(CAFFE_HIP_NUM_THREADS),
         0,
         context_.hip_stream(), 
...
template <>
template <typename T>
void SpatialBNGradientOp<HIPContext>::
  ...
  const T inv_nhw = T(1) / static_cast<T>(N * HxW);
 hipLaunchKernelGGL( ComputeMultiBatchScaleBiasGradientsAndFusedParamsHIPKernel<T>
      , dim3(CAFFE_GET_BLOCKS(C)),
         dim3(CAFFE_HIP_NUM_THREADS),
         0,
         context_.hip_stream(), 
...
template <>
template <typename T>
void SpatialBNGradientOp<HIPContext>::ComputeScaleBiasGradientsAndFusedParams(
    const int N,
    const int C,
    const int HxW,
    const T* dY,
  ...
  if (order_ == StorageOrder::NCHW) {
   hipLaunchKernelGGL( ComputeScaleBiasGradientsAndFusedParamsHIPKernel<T, StorageOrder::NCHW>
        , dim3(std::min(C, CAFFE_MAXIMUM_NUM_BLOCKS)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
            N,
   ...
   hipLaunchKernelGGL( ComputeScaleBiasGradientsAndFusedParamsHIPKernel<T, StorageOrder::NHWC>
        , dim3(std::min(C, CAFFE_MAXIMUM_NUM_BLOCKS)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
...
template <>
template <typename T>
void SpatialBNGradientOp<HIPContext>::ComputeXGradient(
    const int N,
    const int C,
    const int HxW,
    const T* dY,
  ...
  if (order_ == StorageOrder::NCHW) {
   hipLaunchKernelGGL( ComputeXGradientHIPKernel<T, StorageOrder::NCHW>
   ...
   hipLaunchKernelGGL( ComputeXGradientHIPKernel<T, StorageOrder::NHWC>
        , dim3(CAFFE_GET_BLOCKS(size)),
           dim3(CAFFE_HIP_NUM_THREADS),
           0,
           context_.hip_stream(), 
