...
__global__ void ComputeMulGradientHIPKernel(
...
template <typename TGrad, typename TIn, int D>
void ComputeMulGradientHIPImpl(
    const int outer_size,
    const int inner_size,
    const int* Y_dims,
    const int* W_dims,
 ...
 hipLaunchKernelGGL( ComputeMulGradientHIPKernel<TGrad, TIn, D>
...
void ComputeMulGradientHIP(
  ...
  if (outer_size > 0 && inner_size > 0) {
    ...
    math::Set<TGrad, HIPContext>(outer_size, TGrad(0), dX, context);
  }
}
...
bool MulFunctor<HIPContext>::Backward(
  ...
  ComputeMulGradientHIP<TGrad, TIn>(
...
REGISTER_HIP_OPERATOR(
    Mul,
    BinaryElementwiseOp<NumericTypes, HIPContext, MulFunctor<HIPContext>>);
REGISTER_HIP_OPERATOR(
    MulGradient,
    BinaryElementwiseGradientOp<
        NumericTypes,
        HIPContext,
        MulFunctor<HIPContext>>);
