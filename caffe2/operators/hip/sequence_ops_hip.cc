...
template <>
template <typename T>
bool AddPaddingOp<HIPContext>::MakePadding(
    const T* in_ptr,
    T* out_ptr,
 ...
 hipLaunchKernelGGL( AddPaddingKernel<T>
      , dim3(lengths_size), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
          in_ptr,
          static_cast<int>(block_size),
          static_cast<int>(lengths_size),
          static_cast<int>(outer_size),
          lengths_prefix_sum_ptr,
          padding_start_ptr,
          static_cast<int>(startPaddingWidth_),
          padding_end_ptr,
          static_cast<int>(endPaddingWidth_),
          out_ptr,
          lengths_out_ptr);
  return true;
}
...
template <>
template <typename T>
void GatherPaddingOp<HIPContext>::GatherPadding(
  ...
  if (lengths_size > 0) {
   ...
   hipLaunchKernelGGL( gather_padding_kernel<T>
        , dim3(min(block_size, CAFFE_MAXIMUM_NUM_BLOCKS)),
           ...
           0,
           context_.hip_stream(), 
            static_cast<const int>(lengths_size),
            static_cast<const int>(block_size),
            static_cast<const int>(startPaddingWidth_),
            static_cast<const int>(endPaddingWidth_),
            in_ptr,
            lengths_ptr,
