...
__global__ void SliceCopyKernel(
  ...
  if ((copy_size % sizeof(int) == 0) &&
    ...
    HIP_1D_KERNEL_LOOP(index, num_blocks * copyChunks) {
...
bool SliceImplGpu(
  ...
  if (!backward) {
   ...
   hipLaunchKernelGGL( SliceCopyKernel, 
        dim3(::min(num_blocks, CAFFE_MAXIMUM_NUM_BLOCKS)),
        dim3(CAFFE_HIP_NUM_THREADS),
        0,
        context->hip_stream(), 
        src_offset_bytes,
        static_cast<int>(src_block_size_bytes),
        dst_offset_bytes,
        static_cast<int>(static_cast<int>(dst_block_size_bytes)),
        static_cast<int>(static_cast<int>(dst_block_size_bytes)),
        static_cast<int>(itemsize),
   ...
   hipLaunchKernelGGL( SliceCopyKernel, 
        ...
        context->hip_stream(), 
        src_offset_bytes,
        static_cast<int>(static_cast<int>(src_block_size_bytes)),
        dst_offset_bytes,
        static_cast<int>(dst_block_size_bytes),
        static_cast<int>(static_cast<int>(src_block_size_bytes)),
        static_cast<int>(itemsize),
        static_cast<int>(num_blocks));
  }
...
template<>
class SliceOp<HIPContext> : public Operator<HIPContext> {
 public:
  ...
  bool DoRunWithType() {
    ...
    return SliceImplGpu<SIndex, HIPContext>(
...
};  // class SliceOp<HIPContext>

REGISTER_HIP_OPERATOR(Slice, SliceOp<HIPContext>);

template <>
class SliceGradientOp<HIPContext> : public Operator<HIPContext> {
 public:
  USE_OPERATOR_FUNCTIONS(HIPContext);
 ...
 private:
  ...
  Tensor starts_host_{CPU};
  Tensor ends_host_{CPU};
};  // class SliceGradientOp<HIPContext>
REGISTER_HIP_OPERATOR(SliceGradient, SliceGradientOp<HIPContext>);
} // namespace caffe2
