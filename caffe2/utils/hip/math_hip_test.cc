...
void executeGpuBinaryOpTest(
  ...
  auto* tensorx0 = BlobGetMutableTensor(blobx0, HIP);
  auto* tensorx1 = BlobGetMutableTensor(blobx1, HIP);
...
class GemmBatchedGPUTest
 ...
 protected:
  void SetUp() override {
    ...
    X_ = BlobGetMutableTensor(X_blob, HIP);
    W_ = BlobGetMutableTensor(W_blob, HIP);
    Y_ = BlobGetMutableTensor(Y_blob, HIP);
  ...
  void RunGemmBatched(const float alpha, const float beta) {
    ...
    math::GemmBatched<float, HIPContext>(
...
class ReduceTensorGPUTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    X_ = BlobGetMutableTensor(blob_x, HIP);
    Y_ = BlobGetMutableTensor(blob_y, HIP);
  }
  ...
  void SetUpData(
    ...
    hip_context_->CopyFromCPU<float>(
  ...
  void VerifyResult(const std::vector<float>& expected_output) {
    ...
    Y_host->CopyFrom(*Y_, hip_context_.get());
...
class MomentsGPUTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    X_ = BlobGetMutableTensor(blob_x, HIP);
    mean_ = BlobGetMutableTensor(blob_mean, HIP);
    variance_ = BlobGetMutableTensor(blob_variance, HIP);
  }
