...
bool BatchGatherOp<HIPContext>::DoRunWithType() {
 ...
 hipLaunchKernelGGL( BatchGatherKernel, 
      dim3(std::min(M, CAFFE_MAXIMUM_NUM_BLOCKS)),
      dim3(std::min(N * block_size, CAFFE_HIP_NUM_THREADS)),
      0,
      context_.hip_stream(), 
      src_base,
      out,
...
bool BatchGatherGradientOp<HIPContext>::DoRunWithType() {
  return DispatchHelper<
      ...
      TInd>::call(this, OperatorBase::Input<Tensor>(DATA, HIP));
}
