...
void THNN_(TemporalMaxPooling_updateOutput)(
 ...
 hipLaunchKernelGGL( cunn_TemporalMaxPooling_updateOutputKernel<real> ,  dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
...
void THNN_(TemporalMaxPooling_updateGradInput)(
  ...
  dim3 threads(nthreads);
  if (kW <= dW) {
   hipLaunchKernelGGL( cunn_TemporalMaxPooling_updateGradInputKernel<real> ,  dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
   ...
   hipLaunchKernelGGL( cunn_TemporalMaxPooling_updateGradInputKernelAtomic<real> ,  dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
        gradInput_data, gradOutput_data, static_cast<THCIndex_t *>(indices_data), static_cast<int>(input_w), static_cast<int>(input_n), static_cast<int>(output_w), static_cast<int>(kW), static_cast<int>(dW));
  }
  THCudaCheck(hipGetLastError());
