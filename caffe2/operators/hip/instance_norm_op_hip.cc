...
template <>
bool InstanceNormOp<float, HIPContext>::RunOnDeviceWithOrderNHWC() {
...
template <>
bool InstanceNormOp<float, HIPContext>::RunOnDeviceWithOrderNCHW() {
  const auto& input = Input(INPUT);
  const auto& scale = Input(SCALE);
  const auto& bias = Input(BIAS);
  auto output = Output(OUTPUT);
...
template <>
bool InstanceNormGradientOp<float, HIPContext>::RunOnDeviceWithOrderNHWC() {
  const auto& input = Input(INPUT);
  const auto& scale = Input(SCALE);
  const auto& bias = Input(BIAS);
...
template <>
bool InstanceNormGradientOp<float, HIPContext>::RunOnDeviceWithOrderNCHW() {
  const auto& input = Input(INPUT);
  const auto& scale = Input(SCALE);
  const auto& bias = Input(BIAS);
  const auto& output_grad = Input(OUTPUT_GRAD);
...
REGISTER_HIP_OPERATOR(InstanceNorm, InstanceNormOp<float, HIPContext>);
REGISTER_HIP_OPERATOR(
    InstanceNormGradient,
    InstanceNormGradientOp<float, HIPContext>);
} // namespace caffe2
