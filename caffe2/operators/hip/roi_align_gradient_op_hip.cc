...
template <>
bool RoIAlignGradientOp<float, HIPContext>::RunOnDevice() {
  ...
  if (dY.size() > 0) { // Handle possibly empty gradient if there were no rois
   hipLaunchKernelGGL( RoIAlignBackwardFeature<float>
        , dim3(CAFFE_GET_BLOCKS(dY.size())),
           ...
           context_.hip_stream(), 
            ...
            static_cast<const int>(sampling_ratio_),
...
REGISTER_HIP_OPERATOR(
    RoIAlignGradient,
    RoIAlignGradientOp<float, HIPContext>);
} // namespace caffe2
