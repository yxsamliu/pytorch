...
#define HANDLE_CASE(DIMS) \
 hipLaunchKernelGGL( CatArrayBatchedCopy<scalar_t, unsigned int, DIMS>, dim3(catGrid), dim3(applyBlock), 0, THCStream_stream(stream), data, d_inputs, param, static_cast<const int>(dimension), param.outputStride[static_cast<const int>(dimension)]);
  ...
  if (nDimension == 2) {
    ...
    if (size > 0) {
     ...
     hipLaunchKernelGGL( THCTensor_copyFromDiagonal<scalar_t>, dim3(grid), dim3(threads), 0, THCState_getCurrentStream(state), 
      THCTensor_(data)(state, self_), THCTensor_(data)(state, src_), static_cast<ptrdiff_t>(start), static_cast<ptrdiff_t>(size), static_cast<ptrdiff_t>(stride0 + stride1), static_cast<ptrdiff_t>(strideSelf));
    }
  } else {
    ...
    if (size > 0) {
     ...
     hipLaunchKernelGGL( THCTensor_copyToDiagonal<scalar_t>, dim3(grid), dim3(threads), 0, THCState_getCurrentStream(state), 
      THCTensor_(data)(state, self_), THCTensor_(data)(state, src_), static_cast<ptrdiff_t>(start), static_cast<ptrdiff_t>(totalElements), static_cast<ptrdiff_t>(stride0 + stride1), static_cast<ptrdiff_t>(strideSrc));
    }
  }
  THCudaCheck(hipGetLastError());
}
