...
bool TransformOnGPU(
 ...
 hipLaunchKernelGGL( transform_kernel<
    T_IN, T_OUT>, dim3(N), dim3(dim3(16, 16)), 0, context->hip_stream(), 
      static_cast<const int>(N), static_cast<const int>(C), static_cast<const int>(H), static_cast<const int>(W), mean.template data<float>(), std.template data<float>(),
...
template bool TransformOnGPU<uint8_t, float16, HIPContext>(
    Tensor& X,
    Tensor* Y,
    Tensor& mean,
    Tensor& std,
