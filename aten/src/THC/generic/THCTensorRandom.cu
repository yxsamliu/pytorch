  ...
  if (n_sample == 1 && maxShared >= requiredShared) {
   ...
   hipLaunchKernelGGL( sampleMultinomialOnce<real, accreal>
    ...
    if (with_replacement) {
     ...
     hipLaunchKernelGGL( sampleMultinomialWithReplacement<real>
      ...
      for (int sample = 0; sample < n_sample; ++sample) {
       ...
       hipLaunchKernelGGL( sampleMultinomialWithoutReplacement<real>
          , dim3(grid), dim3(block), 0, THCState_getCurrentStream(state), 
            gen->state.gen_states,
            static_cast<int>(n_sample),
...
THC_API void THCTensor_(multinomialAliasSetup)(THCState *state, THCTensor *_probs, THCudaLongTensor *_J, THCTensor *_q){
 ...
 hipLaunchKernelGGL( aliasMultinomialFilter<real>
 ...
 hipLaunchKernelGGL( aliasMultinomialSetup<real>
 ...
 hipLaunchKernelGGL( condDiv<real>, 
    dim3(inputBlockDim), dim3(BLOCK_SIZE), 0, THCState_getCurrentStream(state), 
                      THCTensor_(data)(state, _q),
                      static_cast<int64_t *>(THCudaLongTensor_data(state, _J)),
...
THC_API void THCTensor_(multinomialAliasDraw)(THCState *state, THCudaLongTensor *self, THCudaLongTensor *_J, THCTensor *_q){
 ...
 hipLaunchKernelGGL( multinomialAliasDrawKernel<real>
...
GENERATE_KERNEL2(generate_random, real, int32_t base, uint32_t range, uint32_t, hiprand, \
