...
template <typename Dtype>
void row2col(hipStream_t stream, const Dtype *data_row, const int channels,
  ...
  // Launch
 hipLaunchKernelGGL( row2col_kernel<Dtype>, dim3(GET_BLOCKS(num_kernels)), dim3(CUDA_NUM_THREADS), 0, stream, 
      static_cast<const int>(num_kernels), data_row, static_cast<const int>(width), static_cast<const int>(ksize_w), static_cast<const int>(pad_w), static_cast<const int>(stride_w), static_cast<const int>(1), static_cast<const int>(width_col),
      data_col);
...
template <typename Dtype, typename Acctype>
void col2row(hipStream_t stream, const Dtype *data_col, const int channels,
 ...
 hipLaunchKernelGGL( col2row_kernel<
      Dtype, Acctype>, dim3(GET_BLOCKS(num_kernels)), dim3(CUDA_NUM_THREADS), 0, stream, 
      static_cast<const int>(num_kernels), data_col, static_cast<const int>(width), static_cast<const int>(channels), static_cast<const int>(patch_w), static_cast<const int>(pad_w), static_cast<const int>(stride_w),
      static_cast<const int>(dilation_w), static_cast<const int>(width_col), data_row);
