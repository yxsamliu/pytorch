           ...
           int osizeH)
{
  ...
  while (totalZ > 0) {
   ...
   hipLaunchKernelGGL( cunn_VolumetricAdaptiveMaxPooling_updateOutput_kernel<real>
           ...
           THCIndexTensor *indices)
{
  ...
  while (totalZ > 0) {
    ...
    if (atomic)
    {
     hipLaunchKernelGGL( cunn_atomic_VolumetricAdaptiveMaxPooling_updateGradInput_kernel<real>
        , dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state), 
     ...
     hipLaunchKernelGGL( cunn_VolumetricAdaptiveMaxPooling_updateGradInput_kernel<real>
        , dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state), 
          gradInput_data, gradOutput_data, static_cast<THCIndex_t *>(indices_data),
          static_cast<int>(isizeT), static_cast<int>(isizeH), static_cast<int>(isizeW), static_cast<int>(osizeT), static_cast<int>(osizeH), static_cast<int>(osizeW), static_cast<int64_t>(offsetZ)
        );
