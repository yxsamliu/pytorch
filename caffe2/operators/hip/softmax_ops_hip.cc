...
void Softmax(
  ...
  if (!log_softmax) {
   hipLaunchKernelGGL( SoftmaxNormalizeKernel, 
        ...
        0,
        context->hip_stream(), static_cast<const int>(size), static_cast<const int>(D), probs, scales, probs);
  } else {
   hipLaunchKernelGGL( SoftmaxNormalizeLogsKernel, 
        dim3(CAFFE_GET_BLOCKS(size)),
        dim3(CAFFE_HIP_NUM_THREADS),
        0,
        context->hip_stream(), static_cast<const int>(size), static_cast<const int>(D), logits, rowmax, scales, probs);
  }
}
...
bool SpatialSoftmaxWithLossOp<float, HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( SpatialSoftmaxKernel, 
      ...
      dim3(CAFFE_HIP_NUM_THREADS),
      0,
      context_.hip_stream(), static_cast<const int>(N), static_cast<const int>(D), static_cast<const int>(W), static_cast<const int>(H), Xdata, Pdata);
...
bool SpatialSoftmaxWithLossGradientOp<float, HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( SpatialSoftmaxLossGradientKernel, 
      ...
      0,
      context_.hip_stream(), 
      static_cast<const int>(N), static_cast<const int>(D), static_cast<const int>(W), static_cast<const int>(H), label_data, weights, dX_data, weights_.mutable_data<float>());
...
bool SoftmaxGradientOp<float, HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( softmax_gradient_kernel, 
      ...
      0,
      context_.hip_stream(), static_cast<const int>(D), Y.data<float>(), dY.data<float>(), dX_data);
  return true;
}
