...
// based on "Optimizing Parallel Reduction in HIP" by Mark Harris
...
template <>
bool ChannelStatsOp<HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( ChannelStatsBlockKernel<CAFFE_HIP_NUM_THREADS>
      , dim3(numBlocksTotal), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
          static_cast<int>(N),
          static_cast<int>(C),
          static_cast<int>(valsPerChannel),
          X_arr,
          sumScratch_.mutable_data<float>(),
          sumsqScratch_.mutable_data<float>());

 hipLaunchKernelGGL( ChannelStatsFinalSumsKernel<CAFFE_HIP_NUM_THREADS>
      , dim3(C), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
          static_cast<int>(N),
          static_cast<int>(C),
          static_cast<int>(numBlocksPerChannel),
