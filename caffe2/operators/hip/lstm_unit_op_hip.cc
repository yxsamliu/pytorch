...
template <>
void LSTMUnit<at::Half, HIPContext>(
    int N,
    int D,
    int t,
    ...
    const float forget_bias,
    HIPContext* context) {
 hipLaunchKernelGGL( LSTMUnitKernel<at::Half, float>, 
      dim3(CAFFE_GET_BLOCKS(N * D)),
      dim3(CAFFE_HIP_NUM_THREADS),
      0,
      context->hip_stream(), 
...
template <>
void LSTMUnitGradient<at::Half, HIPContext>(
    int N,
    int D,
    int t,
    const at::Half* C_prev,
    ...
    const float forget_bias,
    HIPContext* context) {
 hipLaunchKernelGGL( LSTMUnitGradientKernel<at::Half, float>, 
      dim3(CAFFE_GET_BLOCKS(N * D)),
      dim3(CAFFE_HIP_NUM_THREADS),
      0,
      context->hip_stream(), 
