...
bool StumpFuncOp<float, float, HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( StumpFuncKernel, dim3(CAFFE_GET_BLOCKS(in.size())), dim3(CAFFE_HIP_NUM_THREADS),
    0, context_.hip_stream(), 
      static_cast<const int>(in.size()), threshold_, low_value_, high_value_, in_data, out_data);
  return true;
}
