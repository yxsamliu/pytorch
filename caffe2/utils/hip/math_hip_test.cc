...
void executeGpuBinaryOpTest(
  ...
  auto* tensorx0 = blobx0->GetMutableTensor(HIP);
  auto* tensorx1 = blobx1->GetMutableTensor(HIP);
...
class GemmBatchedGPUTest
 ...
 protected:
  void SetUp() override {
    ...
    X_ = X_blob->GetMutableTensor(HIP);
    W_ = W_blob->GetMutableTensor(HIP);
    Y_ = Y_blob->GetMutableTensor(HIP);
...
class ReduceTensorGPUTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    X_ = blob_x->GetMutableTensor(HIP);
    Y_ = blob_y->GetMutableTensor(HIP);
  }
...
class MomentsGPUTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    X_ = blob_x->GetMutableTensor(HIP);
    mean_ = blob_mean->GetMutableTensor(HIP);
    variance_ = blob_variance->GetMutableTensor(HIP);
  }
