...
void THCTensor_(potri)(THCState *state, THCTensor *ra_, THCTensor *a, const char *uplo)
{
  ...
  dim3 threads(128);
  if (uplo[0] == 'U') {
   hipLaunchKernelGGL( THCTensor_(copyUpperSymmetric), dim3(blocks), dim3(threads), 0, stream, input_data, static_cast<int>(n), static_cast<int>(len));
  } else {
   hipLaunchKernelGGL( THCTensor_(copyLowerSymmetric), dim3(blocks), dim3(threads), 0, stream, input_data, static_cast<int>(n), static_cast<int>(len));
  }
