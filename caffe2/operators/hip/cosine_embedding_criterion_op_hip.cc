...
bool CosineEmbeddingCriterionOp<HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( CECKernel, dim3(CAFFE_GET_BLOCKS(S.size())), dim3(CAFFE_HIP_NUM_THREADS),
...
bool CosineEmbeddingCriterionGradientOp<HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( CECGradientKernel, dim3(CAFFE_GET_BLOCKS(S.size())), dim3(CAFFE_HIP_NUM_THREADS),
                      0, context_.hip_stream(), 
