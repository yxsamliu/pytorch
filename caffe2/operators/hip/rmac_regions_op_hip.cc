...
bool RMACRegionsOp<HIPContext>::RunOnDevice() {
 ...
 hipLaunchKernelGGL( RMACRegionsKernel, 
      ...
      0,
      context_.hip_stream(), 
      static_cast<const int>(W), static_cast<const int>(H), static_cast<const int>(N), num_rois_.data<int>(), output->template mutable_data<float>());
