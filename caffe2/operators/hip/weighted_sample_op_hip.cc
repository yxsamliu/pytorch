...
template <>
bool WeightedSampleOp<float, HIPContext>::RunOnDevice() {
  CAFFE_ENFORCE_EQ(
      InputSize(),
      OutputSize(),
  ...
  if (batch_size > 0 && weights_dim > 0) {
   ...
   hipLaunchKernelGGL( WeightedSampleKernel, 
        dim3(CAFFE_GET_BLOCKS(batch_size)),
        dim3(CAFFE_HIP_NUM_THREADS),
        0,
        context_.hip_stream(), 
        static_cast<const int>(batch_size),
        static_cast<const int>(weights_dim),
        in_weights_data,
        in_val_data,
        unif_samples_data,
        out_idx_data,
...
REGISTER_HIP_OPERATOR(WeightedSample, WeightedSampleOp<float, HIPContext>);
} // namespace caffe2
