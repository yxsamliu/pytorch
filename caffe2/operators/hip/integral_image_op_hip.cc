...
template <>
bool IntegralImageOp<float, HIPContext>::RunOnDevice() {
  auto& X = Input(0);
  auto* Y = Output(0);
 ...
 hipLaunchKernelGGL( RowPassKernel, 
      ...
      0,
      context_.hip_stream(), 
      static_cast<int>(row_pass_size),
      static_cast<int>(rows_out),
...
template <>
bool IntegralImageGradientOp<float, HIPContext>::RunOnDevice() {
...
REGISTER_HIP_OPERATOR(IntegralImage, IntegralImageOp<float, HIPContext>);
REGISTER_HIP_OPERATOR(
    IntegralImageGradient,
    IntegralImageGradientOp<float, HIPContext>);
