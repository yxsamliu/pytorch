...
bool GatherOp<HIPContext>::DoRunWithType() {
  ...
  // return early when the input is empty, since HIP kernel will fail for
  // empty input.
  if (N <= 0) {
    return true;
  }
 ...
 hipLaunchKernelGGL( GatherKernel, 
      ...
      0,
      context_.hip_stream(), src_base, out, idxs, static_cast<const int>(N), static_cast<const int>(block_size));
  return true;
}
