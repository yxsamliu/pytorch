           ...
           THCTensor *weight)
{
  ...
  else
  {
   ...
   hipLaunchKernelGGL( preluForward<scalar_t>, dim3(GET_BLOCKS(n)), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
           ...
           THCTensor *weight)
{
  ...
  else
  {
    ...
    int nElemsPerSample = nOutputPlane * mapSize;
   hipLaunchKernelGGL( preluBackward<scalar_t>, dim3(GET_BLOCKS(n)), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
